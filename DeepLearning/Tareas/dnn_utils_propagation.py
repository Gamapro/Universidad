...
def accuracy(pred, labels):
    """
    Returns accuracy for given predictions and labels.
    """
    pred = np.squeeze(pred)
    labels = np.squeeze(labels)
    return (np.sum(pred == labels) / labels.shape[0])

def predict(X, y, parameters, show=False):
    """
    ...
    print("Accuracy: "  + str(accuracy(p, y)))
    return p