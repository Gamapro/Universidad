...
uint8_t started = 0;
uint8_t notas[] = {112, 99, 88, 83, 74, 66, 59};
//uint8_t notas[] = {238, 212, 189, 178, 159, 141, 126};
...
void apagar(){
	TCCR0 = 0b00001011; // Preescaler de 64, modo ctc, oc0 activado en toggle
	//           __ -> 01 toggle 10 apagado
	PORTB = 0;
}

void encender(uint8_t nota){
	OCR0 = nota;
	TCCR0 = 0b00011011; // Preescaler de 64, modo ctc, oc0 activado en toggle
}
...
void start(){
	...
	TCNT0 = 0;			// Cuenta el num de ciclos
	TIFR  = 0b00000011;	// Banderas
	TIMSK = 0b00000000;	// Cual interrupcion usar
	OCR0  = 238;			// Tope a comparar
	TCCR0 = 0b00011011;	// Preescaler de 64, modo ctc, oc0 activado en toggle
	//            __ -> 01 toggle 10 apagado
	apagar();
    ...
    while(1){
		...
		if(started){
			for(uint8_t i = 0; i < 7; i++){
				if( isClear(PINA,i) ){
					if(i + 1 == int_serial_data - '0' || i + 1 == int_serial_data){
						encender(notas[i]);
					}
					_delay_ms(50);
					while( isClear(PINA,i) ){}
